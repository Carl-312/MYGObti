> 注：这是项目早期产品草案，里面保留了 15 题、`0.08` 模糊边界和“隐藏祥子独立 flag”这类历史设想。当前运行时实现已切到 `questionedit/questionnewV2.md` 的 `V2.1D` 主线，请不要把本文直接当成现行实现说明。

> 当前现行事实请以 `questionedit/questionnewV2.md` 与 `questionedit/candidates/questionnewV2_1D.md` 为准；本文后续章节全部属于历史草案留档。
...
- 本文中的 `15` 题设计、`0.08` 模糊边界和“隐藏角色独立 flag”都属于早期设想，不再描述现行运行时。
- 如果你是在继续工程实现，请不要沿着本文的 Phase 1/2 叙事回推当前代码；当前代码已直接消费 `questionedit/questionnewV2.md`。
- 如需核对当前主线，请优先读根目录 `README.md`、`docs/ARCHITECTURE.md` 和 `questionedit/README.md`。
