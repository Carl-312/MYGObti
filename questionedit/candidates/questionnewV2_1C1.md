...
- 相对基线改动：
  - 继承 V2.1B：`tieBreakerRule.lambda.priorityPair` 从 `0.12` 调到 `0.14`
  - 保留 `Q18` 的单题 latent 补丁
  - 继承当前冻结版 `V2.1C`
  - 只对 `Q14` 的中间档位 `B` 做 `0.1` 级微调
- 修改目的：
  - 验证把 `Q14.B` 从“偏克制半开”往“更接近中性半开”轻推后，是否能继续提升 `爽世/祥子` 而不伤全局
  - 本轮只测一个中间档位，不与其他题或其他档位联动
...
{ "id": "B", "text": "会先给出一点真实想法，但分寸仍握在自己手里", "delta": [-0.1, 0.0, 0.0] },
