...
- 相对基线改动：
  - 仅将 `tieBreakerRule.lambda.priorityPair` 从 `0.12` 上调到 `0.14`
- 修改目的：
  - 在不改题面、不改主轴、不改 latent 结构的前提下，优先增强 `长崎爽世 vs 丰川祥子` 在 tie-breaker 阶段的分离力度
  - 验证单纯参数级微调能否把重点角色对往上拉，同时把触发率继续控制在健康区间
