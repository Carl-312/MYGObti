...
- 相对基线改动：
  - 继承 V2.1A：`tieBreakerRule.lambda.priorityPair` 从 `0.12` 调到 `0.14`
  - 仅重写 1 道 latent 题 `Q18`
  - `Q18` 继续保持所有选项 `delta` 为 `[0, 0, 0]`
  - `Q18` 只调整题面与 `latentDelta`
- 修改目的：
  - 把“接管局面”和“兜底承担”写得更清楚，减少中档选项的语义重叠
  - 让 `爽世/祥子` 的 latent 均值更贴近各自锚点，观察 pair-only 与轻噪声回收是否一起改善
